...
<div class="container text-center" style="width: 60%; margin-top: 2%">
    ...
    <input type="text" maxlength="200" class="form-control input-lg" id="pesquisa_text" placeholder="Search by Title">

    <div class="pesquisa-control" data-info="<?= Url::toRoute(['user/searchtitle']); ?>"></div>
...
<div class="row">
    <div class="col-xs-12">
        <h2>
            <?= Html::button('Add Tip', ['value' => Url::to(['tips/create?'.'&type=' . $title]), 'class'=>'btn btn-success btn3d', 'id' => 'modalBtnAddTip']) ?>
        </h2>
    </div>
</div>
<div class="row">
            ...
            <?php foreach ( $data as $key => $record) {?>
                <div class="card-item panel", id="box_search">
                    ...
                    <div class="card-foot panel-footer">
                        <?= Html::button('Edit', ['value' => Url::to(['tips/update?id=' . $record->id . '&type=' . $title]), 'class'=>'btn btn-primary btn-xs', 'id' => 'modalBtnEditTip']) ?>

                        <?= Html::button('Delete', ['value' => Url::to(['tips/delete?id='.$record->id.'&response='.null . '&type=' . $title]), 'class'=>'btn btn-danger btn-xs', 'id' => 'modalBtn']) ?>
                    </div>
                </div>
            <?php }?>
        </div>
